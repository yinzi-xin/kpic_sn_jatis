...
flatlist = glob.glob('flat_frames/fluxes/*.fits')
flatdata = []
...
flat_mean_o5 = np.nanmean(flat_mean[5])
flat_mean = np.ravel(flat_mean)
flat_medfilt = signal.medfilt(flat_mean,kernel_size=201)

nulledlist = glob.glob('nulled_frames/fluxes/*.fits')
nulleddata = []
...
nulled_mean_o5 = np.nanmean(nulled_mean[5])
...
plt.ylim([0,500])
plt.fill_betweenx([0,500],2.223,2.27,alpha=0.25)
#plt.fill_betweenx([0,500],1.99,2.03,alpha=0.25)
plt.xlabel('Wavelength (um)',fontsize=16)
plt.ylabel('Flux',fontsize=16)
plt.legend(loc='upper left',fontsize=12)
plt.title('November 12, 2022',fontsize=16)
plt.grid(True)
...
plt.ylim([0,1.1])
plt.fill_betweenx([0,1.1],2.223,2.27,alpha=0.25)
plt.grid(True)
plt.tight_layout()
plt.savefig("null_shape_221112.pdf")